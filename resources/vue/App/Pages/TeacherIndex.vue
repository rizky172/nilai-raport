...
export default {
    ...
    methods: {
        ...
        _search: async function() {
            return await this.$api.Teacher.all(this.search);
        },
        _delete: async function(params) {
            return await this.$api.Teacher.delete(params)
        },
        _restore: async function(id) {
            return await this.$api.Teacher.restore(id);
        },
        // ==========
    }
}
                        ...
                        <wv-row>
                            <wv-col>
                                <router-link
                                    ...
                                    v-if="$ac.hasAccess('teacher_create') && $ac.hasAccess('teacher_full_access')">
                                    <i class="fas fa-plus"></i> Tambah</router-link>
                                <router-link
                                    class="btn btn-danger float-right"
                                    :to="{name: 'teacher', query: {page: 1, deleted: 1}}"
                                    v-if="(search.deleted == 0 || search.deleted == null) && ($ac.hasAccess('teacher_delete') && $ac.hasAccess('teacher_full_access'))">
                                    <i class="fa fa-trash"></i> Sampah</router-link>
                                <router-link
                                    class="btn btn-success float-right"
                                    :to="{name: 'teacher', query: {page: 1, deleted: 0}}"
                                            ...
                                            <tr v-for="(tr, $index) in list" :key="$index">
                                                ...
                                                <td>
                                                    <template v-if="search.deleted == 0 || search.deleted == null">
                                                        <router-link
                                                            v-if="$ac.hasAccess('teacher_read')"
                                                            title="Ubah"
                                                            class="btn btn-warning"
                                                            :to="{name: 'teacher-detail', params: {id: tr.id}}">
                                                            <i class="fas fa-pencil-alt"></i></router-link>
                                                        <button-confirm
                                                            v-if="$ac.hasAccess('teacher_delete')"
                                                            title="Hapus"
                                                            @click="destroy(tr.id)"
